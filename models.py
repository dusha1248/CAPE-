...
class CAPE(nn.Module):
	def __init__(self, embedding_dim, poi_num, vocab_size, label_size):
		...
		"""
			...
			label_size: The Number of Labels in dataste
		"""
		...
		self.label_embedding_in = nn.Embedding(self.label_size, self.embedding_dim)
		self.label_embedding_out = nn.Embedding(self.label_size, self.embedding_dim)
		self.poi_embedding = nn.Embedding(self.poi_num, self.embedding_dim)

		# FF Layer
		self.ff_word = nn.Linear(2 * self.embedding_dim, self.embedding_dim)
		self.ff_label = nn.Linear(2 * self.embedding_dim, self.embedding_dim)
		self.init_weights()
	...
	def init_weights(self):
		...
		self.label_embedding_in.weight.data.uniform_(-1.0, 1.0)
		self.label_embedding_out.weight.data.uniform_(-1.0, 1.0)
		self.ff_word.weight.data.uniform_(-0.1, 0.1)
		self.ff_word.bias.data.fill_(0.0)
		self.ff_label.weight.data.uniform_(-0.1, 0.1)
		self.ff_label.bias.data.fill_(0.0)
	...
	def content_1(self, poi, target, context, num_sampled):
		'''
		...
		embedded_context = self.ff_word(embedded_context)
	...
	def content_2(self, poi, target, context, num_sampled):
		'''
			For Check-in Content Layer
			poi: Given POI
			target: Target Label
		...
		# Embedding Lookup
		embedded_poi_in = self.poi_embedding_in(poi)
		embedded_label_target = self.label_embedding_in(target)
		embedded_label_context = self.label_embedding_out(context)

		# Concat
		embedded_context = torch.cat([embedded_poi_in, embedded_label_target], 1)
		embedded_context = self.ff_label(embedded_context)
		...
		negative_samples = Variable(torch.cuda.FloatTensor(batch_size, num_sampled).
									uniform_(0, self.vocab_size - 1).long())
		embedded_samples = self.label_embedding_out(negative_samples).neg()
